...
# Analysis
loo_wm_t<- as.data.frame(matrix(NA,length(cancers),4,dimnames = list(cancers,c("OR","p","ci_low","ci_high"))))

for(cancer in cancers){
  cat(cancer,"\n")
  mut_matrix_sel<- data_per_cancer_type[[cancer]]$mutations
  HLA_matrix_sel<- data_per_cancer_type[[cancer]]$affinities
  loo_wm_t[cancer,]<- do_logreg(mut_matrix_sel,HLA_matrix_sel,"wm")
}

# Save
saveRDS(loo_wm_t, "results/data/logreg_wm.rds")
